...
#define NOTIFY_PN_BIT_POS			0
#define NOTIFY_PN_MASK				BIT(NOTIFY_PN_BIT_POS)
...
enum fp_action_flags {
		FP_KBP_FLAG_DEVICE_ACTION			= MSB_BIT(0),
		FP_KBP_FLAG_ADDITIONAL_DATA_TO_FOLLOW		= MSB_BIT(1),
};
...
struct msg_action_req {
	uint8_t msg_group;
	uint8_t msg_code;
	union data_info info;
	uint8_t additional_data[MSG_ACTION_REQ_ADDITIONAL_DATA_MAX_LEN];
};

union msg_specific_data {
	struct msg_kbp_req kbp_req;
	struct msg_action_req action_req;
};
...
static int notify_personalized_name(struct bt_conn *conn)
{
	...
	struct bt_gatt_attr *additional_data_attr;
	...
	uint8_t packet[FP_CRYPTO_ADDITIONAL_DATA_HEADER_LEN + pn_len];
	...
	additional_data_attr = bt_gatt_find_by_uuid(fast_pair_svc.attrs, fast_pair_svc.attr_count,
						    BT_UUID_FAST_PAIR_ADDITIONAL_DATA);
	__ASSERT_NO_MSG(additional_data_attr != NULL);

	err = bt_gatt_notify(conn, additional_data_attr, packet, sizeof(packet));
	if (err) {
		LOG_ERR("GATT response notify failed: err=%d", err);
		return err;
	}
					 ...
					 struct net_buf_simple *req)
{
	...
	case FP_MSG_ACTION_REQ:
		...
		if ((parsed_req->fp_flags & FP_KBP_FLAG_DEVICE_ACTION) &&
		    (parsed_req->fp_flags & FP_KBP_FLAG_ADDITIONAL_DATA_TO_FOLLOW)) {
			/* Flags can not be both set at the same time. */
			LOG_ERR("Invalid message flags: %" PRIu8 " (Key-based Pairing)",
				parsed_req->fp_flags);
			return -EINVAL;
		}
		...
		if (!(parsed_req->fp_flags & FP_KBP_FLAG_DEVICE_ACTION) &&
		    !(parsed_req->fp_flags & FP_KBP_FLAG_ADDITIONAL_DATA_TO_FOLLOW)) {
			/* One of two flags has to be set. */
			LOG_ERR("Invalid message flags: %" PRIu8 " (Key-based Pairing)",
				parsed_req->fp_flags);
			return -EINVAL;
		}
		...
		if (parsed_req->fp_flags & FP_KBP_FLAG_DEVICE_ACTION) {
			parsed_req->data.action_req.info.data_len = net_buf_simple_pull_u8(req);
		} else if (parsed_req->fp_flags & FP_KBP_FLAG_ADDITIONAL_DATA_TO_FOLLOW) {
			parsed_req->data.action_req.info.data_id = net_buf_simple_pull_u8(req);
		}
	...
	default:
		LOG_ERR("Unexpected message type: %" PRIu8 " (Key-based Pairing)",
			parsed_req->msg_type);
		return -EINVAL;
	}
					...
					uint8_t *additional_actions)
{
	...
	if (parsed_req->fp_flags & FP_KBP_FLAG_NOTIFY_NAME) {
		if (IS_ENABLED(CONFIG_BT_FAST_PAIR_EXT_PN)) {
			WRITE_BIT(*additional_actions, NOTIFY_PN_BIT_POS, 1);
		} else {
			     ...
			     struct msg_kbp_write *parsed_req)
{
	...
	if (!IS_ENABLED(CONFIG_BT_FAST_PAIR_EXT_PN)) {
		...
		LOG_ERR("Action request not supported (Action request)");
		return -ENOTSUP;
	}

	if (!(parsed_req->fp_flags & FP_KBP_FLAG_ADDITIONAL_DATA_TO_FOLLOW)) {
		LOG_ERR("Unexpected message flags: %" PRIu8 " (Action request)",
			parsed_req->fp_flags);
		return -ENOMSG;
	}

	if (parsed_req->data.action_req.info.data_id != personalized_name_data_id) {
		LOG_ERR("Unexpected Data ID: %" PRIu8 " (Action request)",
			parsed_req->data.action_req.info.data_id);
		return -ENOMSG;
	}
...
static int perform_additional_actions(struct bt_conn *conn, uint8_t additional_actions)
{
	...
	if (additional_actions & NOTIFY_PN_MASK) {

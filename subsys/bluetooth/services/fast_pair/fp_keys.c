...
#define WAIT_FOR_ACCOUNT_KEY_BIT_POS		0
#define WAIT_FOR_ACCOUNT_KEY_MASK		BIT(WAIT_FOR_ACCOUNT_KEY_BIT_POS)
#define WAIT_FOR_PERSONALIZED_NAME_BIT_POS	1
#define WAIT_FOR_PERSONALIZED_NAME_MASK		BIT(WAIT_FOR_PERSONALIZED_NAME_BIT_POS)
...
int fp_keys_store_account_key(const struct bt_conn *conn, const struct fp_account_key *account_key)
{
	...
	if (!(proc->wait_for_mask & WAIT_FOR_ACCOUNT_KEY_MASK)) {
...
int fp_keys_store_personalized_name(const struct bt_conn *conn, const char *pn)
{
	...
	if (!(proc->wait_for_mask & WAIT_FOR_PERSONALIZED_NAME_MASK)) {
	...
	if (!err) {
		...
		LOG_WRN("Store Personalized Name error: err=%d", err);
	}
...
int fp_keys_wait_for_personalized_name(const struct bt_conn *conn)
{
	...
	if ((proc->state == FP_STATE_INITIAL) || (proc->state == FP_STATE_USE_TEMP_KEY)) {
		LOG_WRN("Invalid state during state change");
		return -EINVAL;
	}

	__ASSERT_NO_MSG(!(proc->wait_for_mask & WAIT_FOR_ACCOUNT_KEY_MASK));

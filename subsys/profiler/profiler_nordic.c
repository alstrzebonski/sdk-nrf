...
void profiler_log_encode_uint32(struct log_event_buf *buf, const uint32_t data)
{
...
void profiler_log_encode_int32(struct log_event_buf *buf, const int32_t data)
{
	profiler_log_encode_uint32(buf, (const uint32_t)data);
}

void profiler_log_encode_uint16(struct log_event_buf *buf, const uint16_t data)
{
	__ASSERT_NO_MSG(buf->payload - buf->payload_start + sizeof(data)
			 <= CONFIG_PROFILER_CUSTOM_EVENT_BUF_LEN);
...
void profiler_log_encode_int16(struct log_event_buf *buf, const int16_t data)
{
	profiler_log_encode_uint16(buf, (const uint16_t)data);
}

void profiler_log_encode_uint8(struct log_event_buf *buf, const uint8_t data)
{
	__ASSERT_NO_MSG(buf->payload - buf->payload_start + sizeof(data)
			 <= CONFIG_PROFILER_CUSTOM_EVENT_BUF_LEN);
	*(buf->payload) = data;
...
void profiler_log_encode_int8(struct log_event_buf *buf, const int8_t data)
{
	profiler_log_encode_uint8(buf, (const uint8_t)data);
}
